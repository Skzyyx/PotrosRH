 ...
 * registrar nuevos contratos verificando que no exista uno previo para el mismo
 * empleado.
 *
...
public class ContratoDAO implements IContratoDAO {
    ...
    /**
     * Colección de MongoDB que almacena los documentos de los empleados.
     */
    private final MongoCollection<Empleado> empleadosCollection;

    /**
     * Constructor que inicializa la conexión a la colección de contratos en
     * MongoDB. Obtiene la base de datos a través de la clase Conexion y
     * configura la colección para trabajar con objetos de tipo Contrato.
     */
    public ContratoDAO() {
    ...
    /**
     * Registra un nuevo contrato en la base de datos. Verifica que no exista un
     * contrato previo para el mismo empleado antes de registrarlo.
     *
     * @param contrato El objeto Contrato que se va a registrar
     * @return El contrato registrado con éxito
     * @throws AccesoDatosException Si ya existe un contrato para el mismo
     * empleado o si ocurre un error en la base de datos
     */
    @Override
    public Contrato registrarContrato(Contrato contrato) throws AccesoDatosException {
        try {
            System.out.println(contrato.toString());
            // Verificar si ya existe un contrato para ese empleado
            Bson unwind = Aggregates.unwind("$empleado");
            Bson match = Aggregates.match(Filters.eq("empleado._id", contrato.getEmpleado().getId()));
            AggregateIterable<Contrato> resultado = contratosCollection.aggregate(Arrays.asList(unwind, match));
            if (resultado.iterator().hasNext()) {
                throw new AccesoDatosException("Ya existe un contrato para ese empleado.");
            }

            // Actualizar al empleado en la base (estado + horarios)
            empleadosCollection.updateOne(
                    Filters.eq("_id", contrato.getEmpleado().getId()),
                    Updates.combine(
                            Updates.set("estado", "ACTIVO"),
                            Updates.set("horariosLaborales", contrato.getEmpleado().getHorariosLaborales())
                    )
            );

            // Recuperar el empleado actualizado
            Empleado empleadoActualizado = empleadosCollection.find(
                    Filters.eq("_id", contrato.getEmpleado().getId())
            ).first();

            if (empleadoActualizado == null) {
                throw new AccesoDatosException("No se encontró al empleado después de actualizarlo.");
            }

            // Establecer el empleado actualizado dentro del contrato (como documento embebido)
            contrato.setEmpleado(empleadoActualizado);

            // Insertar el contrato con el empleado embebido actualizado
            ...
            throw new AccesoDatosException("Ocurrió un error al intentar registrar el contrato.");
        }
    }

    /**
     * Obtiene la fecha de inicio del contrato de un empleado.
     *
